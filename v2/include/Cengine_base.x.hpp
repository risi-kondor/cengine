...
// #define COPY_WARNING
#define COPY_WARNING \
  cout << "\e[1mWarning:\e[0m " << classname() << " copied." << endl;
// #define ASSIGN_WARNING
#define ASSIGN_WARNING \
  cout << "\e[1mWarning:\e[0m " << classname() << " assigned." << endl;
// #define CONVERT_WARNING(a,b)
#define CONVERT_WARNING(a, b) \
  cout << "\e[1mWarning:\e[0m " << a << " converted to " << b << "." << endl;
...
#define FCG_UNIMPL()                                          \
  printf("Cengine error: function \"%s\" not implemented.\n", \
         __PRETTY_FUNCTION__);
#define FCG_NOTIMPL()                                         \
  printf("Cengine error: function \"%s\" not implemented.\n", \
         __PRETTY_FUNCTION__);
#define CENGINE_UNIMPL()                                      \
  printf("Cengine error: function \"%s\" not implemented.\n", \
         __PRETTY_FUNCTION__);
// #define GENET_UNIMPL() printf("GEnet error: function \"%s\" not
// implemented.\n",__PRETTY_FUNCTION__);

#define CENGINE_DEPRECATED()                                  \
  printf("Cengine warning: function \"%s\" is deprecated.\n", \
         __PRETTY_FUNCTION__);
// #define FCG_DEPRECATED(message) printf("Warning: %s is
// deprecated.\n",(message));
...
#define CNODE_CREATE() \
  ::Cengine::Cnode_count++;  // cout<<::Cengine::Cnode_count<<endl;
#define CNODE_DESTROY() \
  ::Cengine::Cnode_count--;  // cout<<::Cengine::Cnode_count<<endl;
#define CHANDLE_CREATE() \
  ::Cengine::Chandle_count++;  // cout<<::Cengine::Chandle_count<<endl;
#define CHANDLE_DESTROY() \
  ::Cengine::Chandle_count--;  // cout<<::Cengine::Chandle_count<<endl;
#define COPERATOR_CREATE() \
  ::Cengine::Coperator_count++;  // cout<<::Cengine::Coperator_count<<endl;
#define COPERATOR_DESTROY() \
  ::Cengine::Coperator_count--;  // cout<<::Cengine::Coperator_count<<endl;
#define RSCALARB_CREATE() \
  ::Cengine::RscalarB_count++;  // cout<<::Cengine::RscalarB_count<<endl;
#define RSCALARB_DESTROY() \
  ::Cengine::RscalarB_count--;  // cout<<::Cengine::RscalarB_count<<endl;
#define CSCALARB_CREATE() \
  ::Cengine::CscalarB_count++;  // cout<<::Cengine::CscalarB_count<<endl;
#define CSCALARB_DESTROY() \
  ::Cengine::CscalarB_count--;  // cout<<::Cengine::CscalarB_count<<endl;
#define CTENSORB_CREATE() \
  ::Cengine::             \
      CtensorB_count++;  // {CoutLock lk;
                         // cout<<"CtensorB("<<dims<<","<<nbu<<","<<device<<")"<<endl;}
#define CTENSORB_DESTROY() \
  ::Cengine::CtensorB_count--;  // cout<<::Cengine::CtensorB_count<<endl;
#define CMATRIXB_CREATE() \
  ::Cengine::             \
      CmatrixB_count++;  // {CoutLock lk;
                         // cout<<"CtensorB("<<dims<<","<<nbu<<","<<device<<")"<<endl;}
#define CMATRIXB_DESTROY() \
  ::Cengine::CmatrixB_count--;  // cout<<::Cengine::CtensorB_count<<endl;
#define CTENSORARRAYB_CREATE() \
  ::Cengine::                  \
      CtensorBarray_count++;  // {CoutLock lk;
                              // cout<<"CtensorB("<<dims<<","<<nbu<<","<<device<<")"<<endl;}
#define CTENSORARRAYB_DESTROY() \
  ::Cengine::CtensorBarray_count--;  // cout<<::Cengine::CtensorB_count<<endl;
#else
#define CNODE_CREATE() ;
#define CNODE_DESTROY() ;
#define CHANDLE_CREATE() ;
...
#ifdef _CENGINE_WITH_CUDA
#include <cuda.h>
#include <cuda_runtime.h>
#endif

#ifndef _CENGINGE_WITH_CUDA
...
#define NOCUDA_ERROR \
  cout << "Error: Cengine was compiled without GPU support." << endl;
#define FCG_CPUONLY()                                                \
  if (device > 0) {                                                  \
    printf("Cengine error: CUDA code for \"%s\" not implemented.\n", \
           __PRETTY_FUNCTION__);                                     \
    exit(-1);                                                        \
  }
...
struct fill_pattern {};
struct fill_noalloc : public fill_pattern {
  fill_noalloc() {}
};
...
struct fill_fn : public fill_pattern {
  fill_fn() {}
};
struct fill_ones : public fill_pattern {
  fill_ones() {}
};
struct fill_sequential : public fill_pattern {
  fill_sequential() {}
};
struct fill_identity : public fill_pattern {
  fill_identity() {}
};
struct fill_uniform : public fill_pattern {
  fill_uniform() {}
};
struct fill_tensor : public fill_pattern {
  fill_tensor() {}
};
struct fill_stack : public fill_pattern {
  fill_stack() {}
};
struct fill_cat : public fill_pattern {
  fill_cat() {}
};
...
struct fill_cgaussian : public fill_pattern {
  fill_cgaussian() {}
};
...
template <class TYPE, typename... Args>
void const_variadic_unroller_sub(vector<const TYPE*>& argv,
                                 const TYPE& x,
                                 Args&... args) {
  argv.push_back(&x);
...
void const_derived_variadic_unroller_sub(vector<const TYPE*>& argv,
                                         const TYPE2& x,
                                         Args&... args) {
  argv.push_back(&x);
...
int moveValuesToDeviceIfAnyOnDevice(const vector<OBJ*>& x) {
  ...
  if (dev) {
    for (auto p : x) {
      const_cast<OBJ*>(p)->value->to_device();
    }
  }
  return dev;
}
...
template <typename OBJ>
int moveToDeviceIfAnyOnDevice(const OBJ& x,
                              const OBJ& y,
...
template <class TYPE>
_view<TYPE> view(const TYPE& obj) {
  return obj;
}
...
template <class TYPE>
_like<TYPE> like(const TYPE& obj) {
  return obj;
}
