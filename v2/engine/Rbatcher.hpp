...
class exec_rbatcher_op : public Coperator, public BatcherExecutor {
 public:
  ...
  string str() const {
    ...
    return "exec_rbatcher_op<" +
           dynamic_cast<RbatchedOperator*>(nodes[0]->op)->rbatcher_name() +
...
class Rbatcher : public Rbatcher_base {
 public:
  ...
  Rbatcher(BasicCnodeEngine* _engine) : engine(_engine) {}
  ...
  ~Rbatcher() { DEBUG_ENGINE2("Erasing Rbatcher " << name << " " << id); }
 ...
 public:
  ...
  void release() {  // protected by done_mx
    DEBUG_ENGINE2("    Releasing Rbatcher for " << name << " [" << ready.size()
                                                << "]");
    Cnode* node =
        engine->new_node(new exec_rbatcher_op(engine, std::move(ready)));
    engine->release_batcher(node);
    ready.clear();
  }

  int flush() {  // protected_by done_mx
    DEBUG_ENGINE2("    Flushing Rbatcher " << name << ". " << waiting.size()
                                           << " " << ready.size());
    if (ready.size() > 0) {
      release();
    }
  ...
  int nwaiting() const { return waiting.size(); }
};
