...
class Chandle {
 ...
 public:
  Chandle(Cnode* _node) : node(_node) {
    ...
    // DEBUG_ENGINE({CoutLock lk; cout<<"    Creating handle to
    // "<<node->ident()<<endl;});
  ...
  ~Chandle() {
    // DEBUG_ENGINE({CoutLock lk; cout<<"    Deleting handle to
    // "<<node->ident()<<endl;});
 ...
 public:
  string ident() const { return "H" + to_string(id); }

  string str() const { return ident() + " [" + node->ident() + "]"; }
};
