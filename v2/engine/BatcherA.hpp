...
class exec_batcher_op : public Coperator, public BatcherExecutor {
 public:
  ...
  exec_batcher_op(const vector<Cnode*>& _nodes) : nodes(_nodes){};

  void exec() {
    if (nodes.size() == 0) {
      CoutLock lk;
      cout << "\e[1mEmpty batcher\e[0m" << endl;
      return;
    }
    assert(nodes.size() > 0);
...
class BatcherA : public Batcher {
 public:
  ...
  BatcherA(BasicCnodeEngine* _engine) : engine(_engine) {}
 ...
 public:
  ...
  void release() {  // protected by done_mx
    DEBUG_ENGINE({
      CoutLock lk;
      cout << "  Releasing batcher " << name << " [" << ready.size() << "]"
           << endl;
    });
    // working=true;
  ...
  int flush() {  // protected_by done_mx
    // DEBUG_ENGINE2("    Flushing batcher "<<name<<". "<<waiting.size()<<"
    // "<<ready.size());
    if (ready.size() > 0) {
      release();
    }
