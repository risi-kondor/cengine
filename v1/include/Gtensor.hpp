...
namespace Cengine{
  ...
  public: // ---- Constructors -------------------------------------------------------------------------------
    ...
    Gtensor(const Gdims& _dims, const device_id& dev=0): 
    ...
    Gtensor(const int _k, const Gdims& _dims, const vector<int>_strides, const int _asize, const int _device=0):
      k(_k), dims(_dims), strides(_strides), asize(_asize), device(_device){
      memsize=asize;
      if(device==0) arr=new TYPE[memsize];
      if(device==1) CUDA_SAFE(cudaMalloc((void **)&arrg, memsize*sizeof(TYPE)));
    }

    Gtensor(const int _k, const Gdims& _dims, const vector<int>_strides, const int _asize, const device_id& _dev=0):
      k(_k), dims(_dims), strides(_strides), asize(_asize), device(_dev.id()){
      memsize=asize;
      if(device==0) arr=new TYPE[memsize];
      if(device==1) CUDA_SAFE(cudaMalloc((void **)&arrg, memsize*sizeof(TYPE)));
    }

    Gtensor(const Gdims& _adims, const Gdims& _dims, const device_id& dev=0): 
    ...
    Gtensor(const vector<TYPE>& v, const device_id& dev=0):
  ...
  public: // ---- Filled constructors -------------------------------------------------------------------------
    ...
    Gtensor(const Gdims& _dims, const fill_raw& dummy, const device_id& dev=0): 
      Gtensor(_dims,dev){}

    Gtensor(const Gdims& _dims, const fill_zero& dummy, const device_id& dev=0): 
      Gtensor(_dims,dev) {
      if(dev.id()==0) std::fill(arr,arr+memsize,0);
      if(dev.id()==1) CUDA_SAFE(cudaMemset(arrg,0,memsize*sizeof(TYPE)));
    }

    Gtensor(const Gdims& _dims, const fill_identity& dummy, const device_id& dev=0): 
      Gtensor(_dims) {
      assert(dims.size()==2);
    ...
    Gtensor(const Gdims& _dims, const fill_gaussian& dummy, const device_id& dev=0):
      Gtensor(_dims){
    ...
    Gtensor(const Gdims& _dims, const fill_cgaussian& dummy, const device_id& dev=0):
      Gtensor(_dims){
      normal_distribution<double> distr;
    ...
    Gtensor(const Gdims& _dims, const fill_sequential& dummy, const device_id& dev=0):
      Gtensor(_dims){
      for(int i=0; i<asize; i++) arr[i]=i;
      to_device(dev);
    }

    Gtensor(const Gdims& _dims, const fill_const<TYPE>& dummy, const device_id& dev=0):
      Gtensor(_dims){
      for(int i=0; i<asize; i++) arr[i]=dummy.p;
      to_device(dev);
    }

    Gtensor(const Gdims& _adims, const Gdims& _dims, const fill_zero& dummy, const device_id& dev=0):
      Gtensor(_adims,_dims,dev){
      normal_distribution<double> distr;
      if(dev.id()==0) std::fill(arr,arr+memsize,0);
      if(dev.id()==1) CUDA_SAFE(cudaMemset(arrg,0,memsize*sizeof(TYPE)));
    }

    Gtensor(const Gdims& _adims, const Gdims& _dims, const fill_raw& dummy, const device_id& dev=0):
      Gtensor(_adims,_dims,dev){
    }

    Gtensor(const Gdims& _adims, const Gdims& _dims, const fill_cgaussian& dummy, const device_id& dev=0):
    ...
    Gtensor(const Gdims& _adims, const Gdims& _dims, const fill_gaussian& dummy, const device_id& dev=0):
      Gtensor(_adims,_dims){
      normal_distribution<double> distr;
      for(int i=0; i<asize; i++) arr[i]=distr(rndGen);
      to_device(dev);
  ...
  public: // ---- Copying -------------------------------------------------------------------------------------
    ...
    Gtensor(const Gtensor<TYPE>& x): 
      Gtensor(x.k,x.dims,x.strides,x.asize,x.device){
      ak=x.ak;
      memsize=x.memsize;
      COPY_WARNING;
      if(device==0) std::copy(x.arr,x.arr+memsize,arr);
      if(device==1) CUDA_SAFE(cudaMemcpy(arrg,x.arrg,memsize*sizeof(TYPE),cudaMemcpyDeviceToDevice));  
    }
    ...
    Gtensor(const Gtensor<TYPE>& x, const device_id& dev): 
      Gtensor(x.k,x.dims,x.strides,x.asize,dev){
      ak=x.ak;
      memsize=x.memsize;
      if(device==0){
	if(x.device==0) std::copy(x.arr,x.arr+memsize,arr);
	if(x.device==1) CUDA_SAFE(cudaMemcpy(arr,x.arrg,memsize*sizeof(TYPE),cudaMemcpyDeviceToHost)); 
      }
      if(device==1){
	if(x.device==0) CUDA_SAFE(cudaMemcpy(arrg,x.arr,memsize*sizeof(TYPE),cudaMemcpyHostToDevice));
	if(x.device==1) CUDA_SAFE(cudaMemcpy(arrg,x.arrg,memsize*sizeof(TYPE),cudaMemcpyDeviceToDevice));  
      }
    }
  ...
  public: // ---- Views --------------------------------------------------------------------------------------
    ...
    Gtensor(const view_flag& flag, Gtensor& x, const Gdims& _dims):
      ...
      arr(x.arr), arrg(x.arrg), is_view(true), device(x.device){
      memsize=x.memsize; 
      strides[k-1]=1;
      for(int i=k-2; i>=0; i--)
	strides[i]=strides[i+1]*dims[i+1];
  ...
  public: // ---- Transporters ------------------------------------------------------------------------------
    ...
    const Gtensor<TYPE>& to_device(const device_id& dev) const{
  ...
  public: // ---- Access -------------------------------------------------------------------------------------
    ...
    TYPE operator()(const Gindex& ix) const{
      if(device>0) FCG_CPUONLY();
      FCG_ASSERT(device==0,"Gtensor::operator() not implemented for GPU.\n");
    ...
    TYPE get(const Gindex& ix) const{
      if(device>0) FCG_CPUONLY();
      FCG_ASSERT(device==0,"Gtensor::get(...) not implemented for GPU.\n");
      int t=0; for(int i=0; i<k; i++) t+=ix[i]*strides[i];
      return arr[t];
    }
    ...
    void set(const Gindex& ix, const TYPE& v){
      if(device>0) FCG_CPUONLY();
      FCG_ASSERT(device==0,"Gtensor::set(...) not implemented for GPU.\n");
      int t=0; for(int i=0; i<k; i++) t+=ix[i]*strides[i];
      arr[t]=v;
    }
  ...
  public: // ---- Operations --------------------------------------------------------------------------------
    ...
    Gtensor<TYPE> odot(const Gtensor<TYPE>& x) const{
      FCG_ASSERT(device==0,"GenTensor<k,TYPE>::odot(...) not implemented for GPU.\n");
      assert(dims==x.dims);
      Gtensor<TYPE> R(dims);
      for(int i=0; i<asize; i++) 
	R.arr[i]=arr[i]*x.arr[i];
    ...
    Gtensor<TYPE> odotc(const Gtensor<TYPE>& x) const{
      FCG_ASSERT(device==0,"GenTensor<k,TYPE>::odotc(...) not implemented for GPU.\n");
      assert(dims==x.dims);
      Gtensor<TYPE> R(dims);
      for(int i=0; i<asize; i++) 
    ...
    Gtensor<TYPE> ReLU() const{
      FCG_ASSERT(device==0,"GenTensor<k,TYPE>::ReLU() not implemented for GPU.\n");
      Gtensor<TYPE> R(dims);
      FCG_UNIMPL();
      //for(int i=0; i<asize; i++) 
  ...
  public: // ---- I/O ----------------------------------------------------------------------------------------
    ...
    */
    ...
    string str(const string indent="") const{
      if(device>0) return Gtensor(*this,device_id(0)).str(indent);
      assert(device==0);
      ostringstream oss;
    ...
    Gtensor(const GenTensor<K,TYPE>& x): 
      Gtensor(K,Gdims(x.dims),vector<int>(K),x.asize,x.device){
      for(int i=0; i<k; i++) strides[i]=x.strides[i];
      //COPY_WARNING;
      if(device==0) std::copy(x.arr,x.arr+asize,arr);
      if(device==1) CUDA_SAFE(cudaMemcpy(arrg,x.arrg,asize*sizeof(TYPE),cudaMemcpyDeviceToDevice));  
    }
    */
    ...
    */
    //Gtensor(const Gdims& _dims, const TYPE x, const device_id& dev=0):
    //Gtensor(_dims){
