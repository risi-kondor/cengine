...
class Gtensor {
 ...
 private:
  ...
  void make_strides(const int _ak) {
    ...
    if (ak > 0) {
      strides[ak - 1] = roundup(strides[ak] * dims[ak], 32);
    }
 ...
 public
  ...
  Gtensor(const Gdims& _dims, const fill_zero& dummy, const device_id& dev = 0)
    ...
    if (dev.id() == 1) {
      CUDA_SAFE(cudaMemset(arrg, 0, memsize * sizeof(TYPE)));
    }
  }

  Gtensor(const Gdims& _dims,
  ...
  Gtensor(const Gdims& _dims,
          const fill_gaussian& dummy,
          const device_id& dev = 0,
          typename std::enable_if<is_same<U, complex<float>>::value |
                                  is_same<U, complex<double>>::value>::type = 0)
      : Gtensor(_dims) {
    normal_distribution<double> distr;
    for (int i = 0; i < asize; i++) {
      arr[i] = TYPE(distr(rndGen), distr(rndGen));
 ...
 public
  ...
  Gtensor(const Gtensor<TYPE>& x)
    ...
    if (device == 1) {
      CUDA_SAFE(cudaMemcpy(arrg, x.arrg, memsize * sizeof(TYPE),
  ...
  Gtensor(const Gtensor<TYPE>& x, const device_id& dev)
    ...
    if (device == 0) {
      ...
      if (x.device == 1) {
        CUDA_SAFE(cudaMemcpy(arr, x.arrg, memsize * sizeof(TYPE),
                             cudaMemcpyDeviceToHost));
      }
    }
    if (device == 1) {
      if (x.device == 0) {
        CUDA_SAFE(cudaMemcpy(arrg, x.arr, memsize * sizeof(TYPE),
 ...
 public
  ...
  const Gtensor<TYPE>& to_device(const device_id& dev) const {
    ...
    if (_dev == 0) {
      ...
      CUDA_SAFE(cudaMemcpy(arr, arrg, memsize * sizeof(TYPE),
                           cudaMemcpyDeviceToHost));
      CUDA_SAFE(cudaFree(arrg));
    ...
    if (_dev > 0) {
      ...
      CUDA_SAFE(cudaMemcpy(arrg, arr, memsize * sizeof(TYPE),
                           cudaMemcpyHostToDevice));
      delete[] arr;
 ...
 public:  // k=4 special cases
  ...
  void set(const int i0,
    ...
    FCG_ASSERT(device == 0, "Gtensor::operator() not implemented for GPU.\n");
    assert(k == 4);
    arr[i0 * strides[0] + i1 * strides[1] + i2 * strides[2] + i3 * strides[3]] =
        x;
  }
  ...
  void inc(const int i0,
    ...
    FCG_CPUONLY();
    assert(k == 4);
    arr[i0 * strides[0] + i1 * strides[1] + i2 * strides[2] +
        i3 * strides[3]] += x;
  }
 ...
 public
  ...
  Gtensor<TYPE> odot(const Gtensor<TYPE>& x) const {
    FCG_ASSERT(device == 0,
               "GenTensor<k,TYPE>::odot(...) not implemented for GPU.\n");
    assert(dims == x.dims);
    Gtensor<TYPE> R(dims);
    for (int i = 0; i < asize; i++) {
      R.arr[i] = arr[i] * x.arr[i];
  ...
  Gtensor<TYPE> odotc(const Gtensor<TYPE>& x) const {
    FCG_ASSERT(device == 0,
               "GenTensor<k,TYPE>::odotc(...) not implemented for GPU.\n");
    assert(dims == x.dims);
    Gtensor<TYPE> R(dims);
    for (int i = 0; i < asize; i++) {
  ...
  Gtensor<TYPE> ReLU() const {
    FCG_ASSERT(device == 0,
               "GenTensor<k,TYPE>::ReLU() not implemented for GPU.\n");
    Gtensor<TYPE> R(dims);
    FCG_UNIMPL();
    // for(int i=0; i<asize; i++)
  ...
  */
  ...
  /*
  void adam_update_complex(Gtensor<TYPE>& g, Gtensor<TYPE>& mt, Gtensor<TYPE>&
  vt, const float beta1, const float beta2, const float alpha, const float
  epsilon){ assert(g.asize==asize); assert(mt.asize==asize);
    assert(vt.asize==asize);
  ...
  */
  ...
  void adagrad_update_complex(const Gtensor<complex<BTYPE> >& g,
  Gtensor<complex<BTYPE> >& G, const BTYPE eta, const BTYPE epsilon){
    assert(g.asize==asize);
    assert(G.asize==asize);
  ...
  */
  ...
  string str(const string indent = "") const {
    if (device > 0) {
      return Gtensor(*this, device_id(0)).str(indent);
    }
    assert(device == 0);
    ostringstream oss;
    ...
    if(k>2){
      for(int offs=0; offs<asize; offs+=strides[k-3]){
        ...
        oss<<indent<<"tensor("; for(int i=0; i<k-2; i++) oss<<ix[i]<<",";
    oss<<"*,*)=\n"; for(int i=0; i<dims[k-2]; i++){ oss<<indent<<"  [ "; for(int
    j=0; j<dims[k-1]; j++) if(abs(arr[offs+i*dims[k-1]+j])>10e-5)
