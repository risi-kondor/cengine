...
The four cases correspond to the object being uninitialized, initialized to zero, the identity matrix, 
...
\member{ctensor\_conj\_op(const ctensor\& x)}{Return \m{\wbar{x}}.}
\member{ctensor\_get\_real\_op(const ctensor\& x)}{Return the real part of \m{x}.}
