...
class CmatrixB : public Cobject {
 ...
 public:
  CmatrixB() { CMATRIXB_CREATE(); }

  ~CmatrixB() { CMATRIXB_DESTROY(); }

  string classname() const { return "CmatrixB"; }
 ...
 public
  ...
  CmatrixB(const int _n0,
      ...
      : CmatrixB(_n0, _n1, _nb, _dev) {
    if (dev == 0) {
      std::fill(arr, arr + memsize, 1.0);
    }
    if (dev == 1) {
      CUDA_SAFE(cudaMemset(arrg, 1.0, memsize * sizeof(float)));
    }
  }
  CmatrixB(const int _n0,
           const int _n1,
  ...
  CmatrixB(const int _n0,
           const int _n1,
           std::function<complex<float>(const int i, const int j)> fn,
           const int _dev = 0)
 ...
 public
  ...
  CmatrixB* clone() const { return new CmatrixB(*this, nowarn); }
 ...
 public
  ...
  float re(const int i, const int j) const { return arr[i * n1 + j]; }

  float im(const int i, const int j) const { return arrc[i * n1 + j]; }

  float& re(const int i, const int j) { return arr[i * n1 + j]; }

  float& im(const int i, const int j) { return arrc[i * n1 + j]; }
 ...
 public
  ...
  CmatrixB* conj() const {
    ...
    CUBLAS_SAFE(
        cublasSaxpy(Cengine_cublas, asize, &alpha, arrg, 1, R->arrg, 1));
    CUBLAS_SAFE(
  ...
  CmatrixB* transp() const {
    ...
    CUBLAS_SAFE(cublasSgeam(Cengine_cublas, CUBLAS_OP_T, CUBLAS_OP_N, n0, n1,
                            &alpha, arrgc, n1, &beta, R->arrgc, n0, R->arrgc,
                            n0));
    return R;
  }
  ...
  CmatrixB* herm() const {
    ...
    CUBLAS_SAFE(cublasSgeam(Cengine_cublas, CUBLAS_OP_T, CUBLAS_OP_N, n0, n1,
                            &alpha, arrg, n1, &beta, R->arrg, n0, R->arrg, n0));
    CUBLAS_SAFE(cublasSgeam(Cengine_cublas, CUBLAS_OP_T, CUBLAS_OP_N, n0, n1,
                            &malpha, arrgc, n1, &beta, R->arrgc, n0, R->arrgc,
                            n0));
    return R;
  }
 ...
 public
  ...
  void add(const CmatrixB& x) {
    ...
    if (dev == 1) {
      const float alpha = 1.0;
      CUBLAS_SAFE(
          cublasSaxpy(Cengine_cublas, asize, &alpha, x.arrg, 1, arrg, 1));
      CUBLAS_SAFE(
  ...
  void add_conj(const CmatrixB& x) {
    ...
    if (dev == 1) {
      ...
      CUBLAS_SAFE(
          cublasSaxpy(Cengine_cublas, asize, &alpha, x.arrg, 1, arrg, 1));
      CUBLAS_SAFE(
          cublasSaxpy(Cengine_cublas, asize, &malpha, x.arrgc, 1, arrgc, 1));
    }
  }
 ...
 public
  ...
  void add(const CmatrixB& x, const complex<float> c) {
    ...
    if (dev == 0) {
      for (int i = 0; i < asize; i++) {
        arr[i] += x.arr[i] * cr - x.arrc[i] * ci;
      }
      for (int i = 0; i < asize; i++) {
        arrc[i] += x.arrc[i] * cr + x.arr[i] * ci;
      }
      return;
    }
    if (dev == 1) {
      ...
      CUBLAS_SAFE(
          cublasSaxpy(Cengine_cublas, asize, &mci, x.arrgc, 1, arrg, 1));
      CUBLAS_SAFE(cublasSaxpy(Cengine_cublas, asize, &ci, x.arrg, 1, arrgc, 1));
      CUBLAS_SAFE(
          cublasSaxpy(Cengine_cublas, asize, &cr, x.arrgc, 1, arrgc, 1));
    }
  }
  ...
  void add_conj(const CmatrixB& x, const complex<float> c) {
    ...
    if (dev == 1) {
      ...
      CUBLAS_SAFE(cublasSaxpy(Cengine_cublas, asize, &ci, x.arrg, 1, arrgc, 1));
      CUBLAS_SAFE(
          cublasSaxpy(Cengine_cublas, asize, &mcr, x.arrgc, 1, arrgc, 1));
    }
  }
 ...
 public
     :  // ---- Add times scalar
        // ----------------------------------------------------------------------------
  void add(const CmatrixB& x, const RscalarB y) { add(x, y.val); }

  void add(const CmatrixB& x, const CscalarB y) { add(x, y.val); }
