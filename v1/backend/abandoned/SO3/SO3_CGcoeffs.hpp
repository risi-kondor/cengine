...
class SO3_CGcoeffs : public Gtensor<TYPE> {
 ...
 private:
  TYPE logfact(int n) { return lgamma(n + 1); }

  TYPE plusminus(int k) {
    if (k % 2 == 1) {
      return -1;
    } else {
      return +1;
  ...
  TYPE slowCG(const int m1, const int m2) {
    ...
    TYPE logA = (logfact(l1 + l2 - l) + logfact(l1 - l2 + l) +
                 logfact(-l1 + l2 + l) - logfact(l1 + l2 + l + 1)) /
                2;
    logA += (logfact(l1 + m1) + logfact(l1 - m1) + logfact(l2 + m2) +
             logfact(l2 - m2) + logfact(l + m3) + logfact(l - m3)) /
            2;

    TYPE wigner = 0;
    for (int t = tmin; t <= tmax; t++) {
      TYPE logB = logfact(t) + logfact(t - t1) + logfact(t - t2) +
                  logfact(t3 - t) + logfact(t4 - t) + logfact(t5 - t);
      wigner += plusminus(t) * exp(logA - logB);
    }

    return plusminus(l1 - l2 - m3) * plusminus(l1 - l2 + m) *
           sqrt((TYPE)(2 * l + 1)) * wigner;
  }
};
