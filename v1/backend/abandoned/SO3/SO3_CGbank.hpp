...
class SO3_CGbank {
 ...
 public:
  ...
  ~SO3_CGbank() {
    for (auto p : cgcoeffsf) {
      delete p.second;
    }
    for (auto p : cgcoeffsd) {
      delete p.second;
    }
    for (auto p : cgcoeffsfG) {
      delete p.second;
    }
    for (auto p : cgcoeffsdG) {
      delete p.second;
    }
  }
  ...
  const SO3_CGcoeffs<double>& getd(const CGindex& ix,
  ...
  int getfC(const int l1, const int l2, const int l) {
    ...
    if (cmem_index_tail + 4 * sizeof(int) > CG_CMEM_DATA_OFFS) {
      cerr << "SO3_CGbank: no room to store index entry in constant memory."
           << endl;
      exit(-1);
    }
    int ix_entry[4];
    ...
    if (cmem_data_tail + sizeof(float) * coeffs.asize > CG_CONST_MEM_SIZE) {
      cerr << "SO3_CGbank: no room to store CG matrix in constant memory."
           << endl;
      exit(-1);
    }
    // cout<<l1<<l2<<l<<coeffs.arr[0]<<endl;
    CUDA_SAFE(cudaMemcpyToSymbol(cg_cmem, reinterpret_cast<void*>(coeffs.arr),
                                 coeffs.asize * sizeof(float), cmem_data_tail,
                                 cudaMemcpyHostToDevice));
...
template <>
inline const SO3_CGcoeffs<float>& SO3_CGbank::get<float>(const int l1,
                                                         const int l2,
                                                         const int l) {
...
template <>
inline const SO3_CGcoeffs<double>& SO3_CGbank::get<double>(const int l1,
                                                           const int l2,
                                                           const int l) {
...
template <>
inline const SO3_CGcoeffs<float>& SO3_CGbank::getG<float>(const int l1,
                                                          const int l2,
                                                          const int l) {
...
template <>
inline const SO3_CGcoeffs<double>& SO3_CGbank::getG<double>(const int l1,
                                                            const int l2,
                                                            const int l) {
