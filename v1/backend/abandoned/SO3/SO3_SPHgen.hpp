...
class SO3_SPHgen {
 public:
  ...
  SO3_SPHgen() : L(0), c1({1, 1}, fill::zero), c2({1, 1}, fill::zero) {}
  ...
  Gtensor<float> operator()(const int _L, const float x) {
    ...
    Gtensor<float> R({_L + 1, _L + 1}, fill::raw);
 ...
 private:
  void extend(const int _L) {
    Gtensor<float> newc1({_L + 1, _L + 1}, fill::zero);
    for (int i = 0; i <= L; i++) {
      for (int j = 0; j <= L; j++) {
        newc1(i, j) = c1(i, j);
      }
    ...
    Gtensor<float> newc2({_L + 1, _L + 1}, fill::zero);
    for (int i = 0; i <= L; i++) {
      for (int j = 0; j <= L; j++) {
        newc2(i, j) = c2(i, j);
      }
    ...
    for (int l = L + 1; l <= _L; l++) {
      ...
      c1(l, l) = -sqrt(((float)(2.0 * l + 1)) / (2.0 * l - 1)) *
                 sqrt(1.0 / (2.0 * l) / (2.0 * l - 1)) * (2.0 * l - 1);
    }
