...
namespace Cengine{
  ...
  class ctensorarray_add_Mprod_op: public Coperator, public CumulativeOperator, public InPlaceOperator{
  ...
  public:
    ...
    ctensorarray_add_Mprod_op(Cnode* R, Cnode* A, Cnode* B, const Gdims& _dims1, const Gdims& _dims2):
      Coperator(R,A,B), dims1(_dims1), dims2(_dims2){}

    static string classname(){
      if(Tsel==0) return "ctensorarray_add_Mprod<"+to_string(Csel)+">";
      if(Tsel==1) return "ctensorarray_add_Mprod_TA<"+to_string(Csel)+">";
      if(Tsel==2) return "ctensorarray_add_Mprod_AT<"+to_string(Csel)+">";
    }
  ...
  public:
    ...
    virtual void exec(){
      ...
      //if(Tsel==0) obj.add_Mprod<Csel>(CTENSORARRAYB(inputs[1]),CTENSORARRAYB(inputs[2]));
      //if(Tsel==1) obj.add_Mprod_TA<Csel>(CTENSORARRAYB(inputs[1]),CTENSORARRAYB(inputs[2]));
      //if(Tsel==2) obj.add_Mprod_AT<Csel>(CTENSORARRAYB(inputs[1]),CTENSORARRAYB(inputs[2]));
    }
  ...
  public:
    ...
    string str() const{
      return "ctensorarray_add_Mprod"+inp_str();
    }
