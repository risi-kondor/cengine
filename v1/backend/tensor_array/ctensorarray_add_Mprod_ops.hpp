#ifndef _ctensorarray_add_Mprod_ops
#define _ctensorarray_add_Mprod_ops
...
class ctensorarray_add_Mprod_op : public Coperator,
 ...
 public:
  ...
  ctensorarray_add_Mprod_op(Cnode* R,
                            Cnode* A,
                            Cnode* B,
                            const Gdims& _dims1,
                            const Gdims& _dims2)
  ...
  static string classname() {
    if (Tsel == 0) {
      return "ctensorarray_add_Mprod<" + to_string(Csel) + ">";
    }
    if (Tsel == 1) {
      return "ctensorarray_add_Mprod_TA<" + to_string(Csel) + ">";
    }
    if (Tsel == 2) {
      return "ctensorarray_add_Mprod_AT<" + to_string(Csel) + ">";
    }
  }
 ...
 public:
  virtual void exec() {
    ...
    CtensorArrayB& obj = CTENSORARRAYB(owner);
    // if(Tsel==0)
    // obj.add_Mprod<Csel>(CTENSORARRAYB(inputs[1]),CTENSORARRAYB(inputs[2]));
    // if(Tsel==1)
    // obj.add_Mprod_TA<Csel>(CTENSORARRAYB(inputs[1]),CTENSORARRAYB(inputs[2]));
    // if(Tsel==2)
    // obj.add_Mprod_AT<Csel>(CTENSORARRAYB(inputs[1]),CTENSORARRAYB(inputs[2]));
  }
 ...
 public:
  string str() const { return "ctensorarray_add_Mprod" + inp_str(); }
  ...
  string batcher_name() const{
    if(Tsel==0) return
  "ctensor_add_Mprod<"+to_string(Csel)+">"+signature().str(); if(Tsel==1) return
  "ctensor_add_Mprod_TA<"+to_string(Csel)+">"+signature().str(); if(Tsel==2)
  return "ctensor_add_Mprod_AT<"+to_string(Csel)+">"+signature().str();
  }
  ctensor_Mprod_signature signature() const{
    return ctensor_Mprod_signature(dims1,dims2);}
  Batcher* spawn_batcher() const{
    return new
  MetaBatcher<ctensor_add_Mprod_op,ctensor_Mprod_signature,BatcherA<ctensor_add_Mprod_op<Tsel,Csel>
  > >(inputs[0]->engine);
  }
  */
  ...
  Rbatcher_base* spawn_rbatcher(BasicCnodeEngine* engine) const{
    return new
  MetaRbatcher<ctensor_add_Mprod_op,ctensor_Mprod_signature,Rbatcher>(engine);
  }
  */
};
