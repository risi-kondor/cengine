...
namespace Cengine{
  ...
  public:
    ...
    string describe() const{
      if(nbu>=0) return "CtensorArrayB"+adims.str()+" "+dims.str()+"["+to_string(nbu)+"]";
  ...
  public: // ---- Filled constructors -----------------------------------------------------------------------
    ...
    CtensorArrayB(const Gdims& _adims, const Gdims& _dims, const FILLTYPE& fill, const int dev=0):
      CFtensorArray(_adims,_dims,fill,dev), dims(_dims){
      CTENSORARRAYB_CREATE();
    }
	  
    template<typename FILLTYPE, typename = typename std::enable_if<std::is_base_of<fill_pattern, FILLTYPE>::value, FILLTYPE>::type>
    CtensorArrayB(const Gdims& _adims, const Gdims& _dims, const int _nbu, const FILLTYPE& fill, const int dev=0):
      CFtensorArray(_adims,_dims.prepend(_nbu),fill,dev), dims(_dims), nbu(_nbu){
      CTENSORARRAYB_CREATE();
    }
	  
    CtensorArrayB(const Gdims& _adims, const Gdims& _dims, const int _nbu, const fill_gaussian& fill, const float c, const int dev=0):
      CFtensorArray(_adims,_dims.prepend(_nbu),fill,c,dev), dims(_dims), nbu(_nbu){
      CTENSORARRAYB_CREATE();
    }

    /*
    CtensorArrayB(const Gdims& _adims, const Gdims& _dims, const int _nbu, std::function<complex<float>(const int i, const int j)> fn, const int dev=0):
    ...
    */
    ...
    /*
    CtensorArrayB(const CtensorArrayB& x, std::function<complex<float>(const complex<float>)> fn):
      CFtensor(x,fn), adims(x.adims), dims(x.dims){
      CTENSORARRAYB_CREATE();
    }
    */

    CtensorArrayB(const CtensorArrayB& x, std::function<complex<float>(const int i, const int j, const complex<float>)> fn):
  ...
  public: // ---- Copying -----------------------------------------------------------------------------------
    ...
    CtensorArrayB* clone() const{
      return new CtensorArrayB(*this, nowarn);
    }
  ...
  public: // ---- Access -------------------------------------------------------------------------------------
    ...
    void copy_into_cell(const Gindex& aix, const CtensorArrayB& x, const Gindex& xaix){
      assert(x.asize==asize);
      assert(nbu==x.nbu);
    ...
    void add_into_cell(const Gindex& aix, const CtensorArrayB& x, const Gindex& xaix){
      assert(x.asize==asize);
      assert(nbu==x.nbu);
      int t=aix(astrides)*cellstride;
      int xt=xaix(x.astrides)*x.cellstride;
