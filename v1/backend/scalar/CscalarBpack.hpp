...
class CscalarBpack {
 public:
  ...
  CscalarBpack(const int _N, const int _nbu = -1) : N(_N), nbu(_nbu) {
    assert(nbu == -1);
  ...
  CscalarBpack(const vector<Cnode*>& nodes, const int s)
    ...
    CUDA_SAFE(
        cudaMemcpy(arrg, arr, 2 * N * sizeof(float), cudaMemcpyHostToDevice));
  }
  ...
  ~CscalarBpack() {
    ...
    float arr[2 * N];
    CUDA_SAFE(
        cudaMemcpy(arr, arrg, 2 * N * sizeof(float), cudaMemcpyDeviceToHost));
    for (int i = 0; i < N; i++) {
