...
class CscalarB : public Cobject {
 public:
  ...
  string classname() const { return "CscalarB"; }

  string describe() const {
    if (nbu >= 0) {
      return "CscalarB[" + to_string(nbu) + "]";
    }
    return "CscalarB";
  }
 ...
 public
     :  // ---- Filled constructors
        // -----------------------------------------------------------------------
  CscalarB(const complex<float> c, const device_id& dev = 0) : val(c) {
    CSCALARB_CREATE();
  }
  ...
  CscalarB(const fill_zero& fill, const device_id& dev = 0) : val(0) {
    CSCALARB_CREATE();
  }
  ...
  CscalarB(const int _nbu,
    ...
    if (nbu == -1) {
      ...
      for (int i = 0; i < nbu; i++) {
        arr[i] = complex<float>(distr(rndGen) * c, distr(rndGen)) * c;
      }
    }
  }
 ...
 public
     :  // ---- Copying
        // -----------------------------------------------------------------------------------
  CscalarB(const CscalarB& x) : nbu(x.nbu) {
    if (nbu == -1) {
      val = x.val;
    } else {
      reallocate();
 ...
 public
  ...
  operator vector<complex<float>>() {
    if (nbu == -1) {
      return vector<complex<float>>(1, val);
    }
 ...
 public
  ...
  void add_to_real(const RscalarB& x) {
    assert(nbu == x.nbu);
    if (nbu == -1) {
      val += complex<float>(x.val, 0);
    } else {
      for (int i = 0; i < nbu; i++) {
        arr[i] += complex<float>(x.arr[i], 0);
      }
    }
  }
  ...
  void add_to_imag(const RscalarB& x) {
    assert(nbu == x.nbu);
    if (nbu == -1) {
      val += complex<float>(0, x.val);
    } else {
      for (int i = 0; i < nbu; i++) {
        arr[i] += complex<float>(0, x.arr[i]);
      }
    }
  }
  ...
  void add_prodcc(const CscalarB& x, const CscalarB& y) {
    if (nbu == -1) {
      val += std::conj(x.val * y.val);
    } else {
      for (int i = 0; i < nbu; i++) {
        arr[i] += std::conj(x.arr[i] * y.arr[i]);
      }
    }
  }
  ...
  void add_div_back1(const CscalarB& g, const CscalarB& x, const CscalarB& y) {
    if (nbu == -1) {
      val -= g.val * std::conj(x.val * complex<float>(pow(y.val, -2.0)));
    } else {
      for (int i = 0; i < nbu; i++) {
        arr[i] -= g.arr[i] *
                  std::conj(x.arr[i] * complex<float>(pow(y.arr[i], -2.0)));
      }
    }
  }
  ...
  void add_abs_back(const CscalarB& g, const CscalarB& x) {
    if (nbu == -1) {
      val +=
          complex<float>(ifthen(x.val.real() > 0, g.val.real(), -g.val.real()),
                         ifthen(x.val.imag() > 0, g.val.imag(), -g.val.imag()));
    } else {
      for (int i = 0; i < nbu; i++) {
        float re = g.arr[i].real();
        float im = g.arr[i].imag();
        arr[i] += complex<float>(ifthen(x.arr[i].real() > 0, re, -re),
                                 ifthen(x.arr[i].imag() > 0, im, -im));
      }
    }
  }
  ...
  void add_pow_back(const CscalarB& g,
                    const CscalarB& x,
                    const float p,
                    const complex<float> c = 1.0) {
    if (nbu == -1) {
      val += c * g.val * std::conj(std::pow(x.val, p));
    } else {
      for (int i = 0; i < nbu; i++) {
        arr[i] += c * g.arr[i] * std::conj(std::pow(x.arr[i], p));
      }
    }
  }
  ...
  void add_ReLU(const CscalarB& x, const float c) {
    if (nbu == -1) {
      float re = x.val.real();
      float im = x.val.imag();
      val += complex<float>(ifthen(re > 0, re, c * re),
                            ifthen(im > 0, im, c * im));
    } else {
      for (int i = 0; i < nbu; i++) {
        float re = x.arr[i].real();
        float im = x.arr[i].imag();
        arr[i] += complex<float>(ifthen(re > 0, re, c * re),
                                 ifthen(im > 0, im, c * im));
      }
    }
  }
  ...
  void add_ReLU_back(const CscalarB& g, const CscalarB& x, const float c) {
    if (nbu == -1) {
      float re = g.val.real();
      float im = g.val.imag();
      val += complex<float>(ifthen(x.val.real() > 0, re, c * re),
                            ifthen(x.val.imag() > 0, im, c * im));
    } else {
      for (int i = 0; i < nbu; i++) {
        float re = g.arr[i].real();
        float im = g.arr[i].imag();
        arr[i] += complex<float>(ifthen(x.arr[i].real() > 0, re, c * re),
                                 ifthen(x.arr[i].imag() > 0, im, c * im));
      }
    }
  }
  ...
  void add_sigmoid(const CscalarB& x) {
    if (nbu == -1) {
      val += complex<float>(1.0 / (1.0 + std::exp(-x.val.real())),
                            1.0 / (1.0 + std::exp(-x.val.imag())));
    } else {
      for (int i = 0; i < nbu; i++) {
        arr[i] += complex<float>(1.0 / (1.0 + std::exp(-x.arr[i].real())),
                                 1.0 / (1.0 + std::exp(-x.arr[i].imag())));
      }
    }
  }
  ...
  void add_sigmoid_back(const CscalarB& g, const CscalarB& x) {
    if (nbu == -1) {
      val += complex<float>(x.val.real() * (1.0 - x.val.real()) * g.val.real(),
                            x.val.imag() * (1.0 - x.val.imag()) * g.val.imag());
    } else {
      for (int i = 0; i < nbu; i++) {
        arr[i] += complex<float>(
            x.arr[i].real() * (1.0 - x.arr[i].real()) * g.arr[i].real(),
