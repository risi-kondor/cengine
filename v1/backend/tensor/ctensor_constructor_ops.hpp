...
class new_ctensor_op : public Coperator {
 public:
  ...
  string str() const { return "ctensor" + dims.str(); }
};
...
class new_ctensor_zero_op : public Coperator {
 public:
  ...
  new_ctensor_zero_op(const Gdims& _dims,
                      const int _nbu = -1,
                      const int _device = 0)
      : dims(_dims), nbu(_nbu), device(_device) {}
  ...
  string str() const { return "ctensor_zero" + dims.str(); }
};
...
class new_ctensor_ones_op : public Coperator {
 public:
  ...
  new_ctensor_ones_op(const Gdims& _dims,
                      const int _nbu = -1,
                      const int _device = 0)
      : dims(_dims), nbu(_nbu), device(_device) {}
  ...
  string str() const { return "ctensor_ones" + dims.str(); }
};
...
class new_ctensor_identity_op : public Coperator {
 public:
  ...
  new_ctensor_identity_op(const Gdims& _dims,
                          const int _nbu = -1,
                          const int _device = 0)
      : dims(_dims), nbu(_nbu), device(_device) {}
  ...
  string str() const { return "ctensor_identity" + dims.str(); }
};
...
class new_ctensor_sequential_op : public Coperator {
 public:
  ...
  new_ctensor_sequential_op(const Gdims& _dims,
                            const int _nbu = -1,
                            const int _device = 0)
      : dims(_dims), nbu(_nbu), device(_device) {}
  ...
  string str() const { return "ctensor_sequential" + dims.str(); }
};
...
class new_ctensor_gaussian_op : public Coperator {
 public:
  ...
  new_ctensor_gaussian_op(const Gdims& _dims,
                          const int _nbu = -1,
                          const int _device = 0)
      : dims(_dims), nbu(_nbu), device(_device) {}
  ...
  string str() const { return "ctensor_gaussian" + dims.str(); }
};
...
class new_ctensor_from_gtensor_op : public Coperator {
 public:
  ...
  new_ctensor_from_gtensor_op(const Gtensor<complex<float>>& _x,
                              const int _nbu = -1,
                              const int _device = 0)
  ...
  string str() const { return "ctensor()"; }
};

class ctensor_copy_op : public Coperator {
 public:
  ctensor_copy_op(Cnode* x) : Coperator(x) {}

  virtual void exec() {
    assert(!owner->obj);
    owner->obj =
        new CtensorB(asCtensorB(inputs[0], __PRETTY_FUNCTION__), nowarn);
  }

  string str() const { return "ctensor_copy(" + inputs[0]->ident() + ")"; }
};
...
class new_ctensor_fn2_op : public Coperator {
 public:
  ...
  new_ctensor_fn2_op(const Gdims& _dims,
                     const int _nbu,
                     function<complex<float>(const int, const int)> _fn,
                     const int _dev = 0)
  ...
  string str() const { return "ctensor_fn2" + dims.str(); }
};
...
class ctensor_apply_op : public Coperator {
 public:
  ...
  string str() const { return "ctensor_apply" + inp_str(); }
};
