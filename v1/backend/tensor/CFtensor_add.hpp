...
void add_sum(const vector<CFtensor*> v) {
  ...
  for (int i = 0; i < N; i++) {
    ...
    CUBLAS_SAFE(
        cublasSaxpy(Cengine_cublas, asize, &alpha, o.arrgc, 1, arrgc, 1));
    cudaDeviceSynchronize();
  }
}
...
void add_to_slice(const CFtensor& x, const int ix, const int offs) {
  ...
  assert(x.device == device);
  for (int i = 0; i < ix; i++) {
    assert(dims[i] == x.dims[i]);
  }
  for (int i = ix; i < x.k; i++) {
    assert(dims[i + 1] == x.dims[i]);
  }
  int subsize = x.asize;
  if (ix > 0) {
    subsize = x.strides[ix - 1];
  }
  int supsize = x.asize / subsize;
  int jstride = asize;
  if (ix > 0) {
    jstride = strides[ix - 1];
  }
...
void add_to_chunk(const CFtensor& x, const int ix, const int offs) {
  ...
  FCG_UNIMPL();
  // const float alpha = 1.0;
  // CUBLAS_SAFE(cublasSaxpy(Cengine_cublas, asize, &alpha, x.arrg, 1, arrg,
  // 1)); CUBLAS_SAFE(cublasSaxpy(Cengine_cublas, asize, &alpha, x.arrgc, 1,
  // arrgc, 1));
}
...
void add_slice(const CFtensor& x, const int ix, const int offs) {
  ...
  for (int i = ix; i < k; i++) {
    assert(x.dims[i + 1] == dims[i]);
  }
  int subsize = asize;
  if (ix > 0) {
    subsize = strides[ix - 1];
  }
  int supsize = asize / subsize;
  int jstride = x.asize;
  if (ix > 0) {
    jstride = x.strides[ix - 1];
  }
...
void add_chunk(const CFtensor& x, const int ix, const int offs, const int n) {
  ...
  if (device == 0) {
    for (int j = 0; j < supsize; j++) {
      for (int m = 0; m < n; m++) {
        for (int i = 0; i < subsize; i++) {
          arr[j * jstride + m * strides[ix] + i] +=
              x.arr[j * jxstride + (m + offs) * x.strides[ix] + i];
          arrc[j * jstride + m * strides[ix] + i] +=
              x.arrc[j * jxstride + (m + offs) * x.strides[ix] + i];
        }
      }
    }
    return;
      ...
      for (int j = 0; j < J; j++) {
        ...
        for (int i = 0; i < I; i++) {
          t += x.arr[offs + i * J + j] * x.arr[offs + i * J + j] +
               x.arrc[offs + i * J + j] * x.arrc[offs + i * J + j];
        }
        arr[a * J + j] += sqrt(t);
      }
    }
...
void add_col_norms_back(const CFtensor& G,
                        const CFtensor& X,
                        const CFtensor& N) {
  ...
  if (device == 0) {
    for (int a = 0; a < A; a++) {
      ...
      for (int j = 0; j < J; j++) {
        ...
        // complex<float> z=complex<float>(1,0)/n/n;
        // //complex<float>(G.arr[a*J+j],G.arrc[a*J+j])/n/n;
        for (int i = 0; i < I; i++) {
          complex<float> u = complex<float>(G.arr[offs + i * J + j],
                                            G.arrc[offs + i * J + j]) /
                             n;
          // complex<float>
          // u=z*complex<float>(G.arr[offs+i*J+j],G.arrc[offs+i*J+j])*
...
void add_divide_cols_back1(const CFtensor& G,
                           const CFtensor& X,
                           const CFtensor& N) {
  assert(G.device == device);
  assert(X.device == device);
    ...
    for (int a = 0; a < A; a++) {
      ...
      for (int j = 0; j < J; j++) {
        ...
        for (int i = 0; i < I; i++) {  // improve
          complex<float> t = complex<float>(G.arr[offs + i * J + j],
                                            G.arrc[offs + i * J + j]) *
                             complex<float>(X.arr[offs + i * J + j],
                                            -X.arrc[offs + i * J + j]) *
                             z;
