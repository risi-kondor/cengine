...
class CtensorBpack {
 public:
  ...
  CtensorBpack(const vector<CtensorB*>& v) : CtensorBpack(v.size(), *v[0]) {
    memsize = v[0]->memsize;
    pack.resize(N);
    for (int i = 0; i < N; i++) {
      pack[i] = v[i];
  ...
  CtensorBpack(const vector<Cnode*>& v, const int s)
      : CtensorBpack(v.size(),
                     *dynamic_cast<CtensorB*>(v[0]->op->inputs[s]->obj)) {
    pack.resize(N);
    for (int i = 0; i < N; i++) {
  ...
  ~CtensorBpack() {
    if (parr) {
      CUDA_SAFE(cudaFree(parr));
    }
    if (parrc) {
      CUDA_SAFE(cudaFree(parrc));
    }
  }
 ...
 public
  ...
  void add_inp_into(CscalarBpack& R, CtensorBpack& Y) { CENGINE_UNIMPL(); }
...
/*
CtensorBpack(const int N, const Gdims& _dims, const int _nbu, const fill_raw&
dummy, const int dev=0): dims(_dims), nbu(_nbu), device(dev){ assert(dev==1);
}
*/

/*
CtensorBpack(const int _N, const CtensorB& model, const fill_raw& dummy, const
int dev=1): dims(model.dims), nbu(model.nbu), device(dev), N(_N){
  assert(dev==1);
  int cst=model.cst;
