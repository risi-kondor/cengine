...
void add_Mprod(const CFtensor& x,
               const CFtensor& y,
               const int nx = 1,
               const int ny = 1) {
  if (x.asize == 0 || y.asize == 0) {
    return;
  }
  ...
  if (device > 0) {
    ...
    CUBLAS_SAFE(cublasSgemm(Cengine_cublas, CUBLAS_OP_N, CUBLAS_OP_N, J, I, K,
                            &alpha1, y.arrgc, J, x.arrgc, K, &beta, arrg, J));
    CUBLAS_SAFE(cublasSgemm(Cengine_cublas, CUBLAS_OP_N, CUBLAS_OP_N, J, I, K,
                            &alpha2, y.arrgc, J, x.arrg, K, &beta, arrgc, J));
    CUBLAS_SAFE(cublasSgemm(Cengine_cublas, CUBLAS_OP_N, CUBLAS_OP_N, J, I, K,
                            &alpha3, y.arrg, J, x.arrgc, K, &beta, arrgc, J));
...
template <int selector>
void add_Mprod_AT(const CFtensor& x,
                  const CFtensor& y,
                  const int nx = 1,
                  const int ny = 1) {
  if (x.asize == 0 || y.asize == 0) {
  ...
  if (device > 0) {
    ...
    CUBLAS_SAFE(cublasSgemm(Cengine_cublas, CUBLAS_OP_T, CUBLAS_OP_N, J, I, K,
                            &alpha1, y.arrgc, K, x.arrgc, K, &beta, arrg, J));
    CUBLAS_SAFE(cublasSgemm(Cengine_cublas, CUBLAS_OP_T, CUBLAS_OP_N, J, I, K,
                            &alpha2, y.arrgc, K, x.arrg, K, &beta, arrgc, J));
    CUBLAS_SAFE(cublasSgemm(Cengine_cublas, CUBLAS_OP_T, CUBLAS_OP_N, J, I, K,
                            &alpha3, y.arrg, K, x.arrgc, K, &beta, arrgc, J));
    cudaDeviceSynchronize();
  }
}
...
template <int selector>
void add_Mprod_TA(const CFtensor& x,
                  const CFtensor& y,
                  const int nx = 1,
                  const int ny = 1) {
  if (x.asize == 0 || y.asize == 0) {
  ...
  // if(y.combined_size(0,ny)!=K){CoutLock lk; cout<<K<<"
  // "<<y.combined_size(0,ny)<<endl;};
  assert(y.combined_size(0, ny) == K);
  ...
  if (device > 0) {
    ...
    CUBLAS_SAFE(cublasSgemm(Cengine_cublas, CUBLAS_OP_N, CUBLAS_OP_T, J, I, K,
                            &alpha1, y.arrgc, J, x.arrgc, I, &beta, arrg, J));
    CUBLAS_SAFE(cublasSgemm(Cengine_cublas, CUBLAS_OP_N, CUBLAS_OP_T, J, I, K,
                            &alpha2, y.arrgc, J, x.arrg, I, &beta, arrgc, J));
    CUBLAS_SAFE(cublasSgemm(Cengine_cublas, CUBLAS_OP_N, CUBLAS_OP_T, J, I, K,
                            &alpha3, y.arrg, J, x.arrgc, I, &beta, arrgc, J));
    cudaDeviceSynchronize();
  }
}
