...
class CtensorB : public Cobject, public CFtensor {
 public:
  ...
  CtensorB() { CTENSORB_CREATE(); }

  ~CtensorB() { CTENSORB_DESTROY(); }

  string classname() const { return "CtensorB"; }

  string describe() const {
    if (nbu >= 0) {
      return "CtensorB" + dims.str() + "[" + to_string(nbu) + "]";
    }
 ...
 public
     :  // ---- Copying
        // -----------------------------------------------------------------------------------
  CtensorB(const CtensorB& x) : CFtensor(x), dims(x.dims), nbu(x.nbu) {
    COPY_WARNING;
    CTENSORB_CREATE();
  }
  ...
  CtensorB* clone() const { return new CtensorB(*this, nowarn); }

 public
     :  // ---- Conversions
        // -------------------------------------------------------------------------------
  CtensorB(const CFtensor& x) : CFtensor(x), dims(x.dims), nbu(-1) {
    CTENSORB_CREATE();
  }

  CtensorB(CFtensor&& x) : CFtensor(std::move(x)), dims(x.dims), nbu(-1) {
    CTENSORB_CREATE();
  }

  void to_device(const int _dev) const { CFtensor::to_device(_dev); }
 ...
 public
     :  // ---- Operations
        // ---------------------------------------------------------------------------------
  CtensorB* conj() const { return new CtensorB(CFtensor::conj()); }

  CtensorB* transp() const { return new CtensorB(CFtensor::transp()); }

  CtensorB* herm() const { return new CtensorB(CFtensor::herm()); }
      ...
      if (x.nbu == -1) {
        ...
        for (int i = 0; i < r.nbu; i++) {
          ...
          for (int j = 0; j < x.nbu; j++) {
            t +=
                complex<float>(arr[i * x.nbu + j], arrc[i * x.nbu + j]) * x.val;
          }
          r.arr[i] = t;
        }
      }
