  public: // ---- Optimization kernels ------------------------------------------------------------------------
    ...
    void adam_update(CFtensor& g, CFtensor& mt, CFtensor& vt, 
	...
	for(int i=0; i<asize; i++){  
	  mt.arr[i]=beta1*mt.arr[i]+g.arr[i]*(static_cast<float>(1.0)-beta1);
	  vt.arr[i]=beta2*vt.arr[i]+std::pow(g.arr[i],2)*(static_cast<float>(1.0)-beta2);  
    ...
    void adam_update_complex(CFtensor& g, CFtensor& mt, CFtensor& vt, 
      const float beta1, const float beta2, const float alpha, const float epsilon){
    ...
    void adagrad_update_complex(const Gtensor<complex<Bfloat> >& g, Gtensor<complex<Bfloat> >& G, 
      const Bfloat eta, const Bfloat epsilon){
    /*
      assert(g.asize==asize);
      assert(G.asize==asize);
