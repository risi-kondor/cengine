    ...
    void add_prod(const CFtensor& x, const CFtensor& y, const CFtensorProductType& T){
      if(device>0) {add_prodG<selector>(x,y,T); return;}
    ...
    template<int selector>
    void add_prodG(const CFtensor& x, const CFtensor& y, const CFtensorProductType& T){
      tuple<int,int,int,int> Ttype=T.type();
      x.to_device(device);
      y.to_device(device);
	...
	if(T.is_xout(0,0)&&T.is_yout(1,1)&&T.is_contr(1,0)){
	  ...
	  cublasSgemm(GEnet_cublas,CUBLAS_OP_N,CUBLAS_OP_N,J1,I1,P1,&alpha0,
	    y.arrg,y.strides[0],x.arrg,x.strides[0],&beta,arrg,strides[0]); 
	  cublasSgemm(GEnet_cublas,CUBLAS_OP_N,CUBLAS_OP_N,J1,I1,P1,&alpha1,
	    y.arrgc,y.strides[0],x.arrgc,x.strides[0],&beta,arrg,strides[0]); 
	  cublasSgemm(GEnet_cublas,CUBLAS_OP_N,CUBLAS_OP_N,J1,I1,P1,&alpha2,
	    y.arrgc,y.strides[0],x.arrg,x.strides[0],&beta,arrgc,strides[0]); 
	  cublasSgemm(GEnet_cublas,CUBLAS_OP_N,CUBLAS_OP_N,J1,I1,P1,&alpha3,
	    y.arrg,y.strides[0],x.arrgc,x.strides[0],&beta,arrgc,strides[0]); 
#endif
	  return;
	}
	...
	if(T.is_xout(0,1)&&T.is_yout(0,0)&&T.is_contr(1,1)){
	  ...
	  cublasSgemm(GEnet_cublas,CUBLAS_OP_T,CUBLAS_OP_N,J1,I1,P1,&alpha0,
	    x.arrg,x.strides[0],y.arrg,y.strides[0],&beta,arrg,strides[0]); 
	  cublasSgemm(GEnet_cublas,CUBLAS_OP_T,CUBLAS_OP_N,J1,I1,P1,&alpha1,
	    x.arrgc,x.strides[0],y.arrgc,y.strides[0],&beta,arrg,strides[0]); 
	  cublasSgemm(GEnet_cublas,CUBLAS_OP_T,CUBLAS_OP_N,J1,I1,P1,&alpha2,
	    x.arrgc,x.strides[0],y.arrg,y.strides[0],&beta,arrgc,strides[0]); 
	  cublasSgemm(GEnet_cublas,CUBLAS_OP_T,CUBLAS_OP_N,J1,I1,P1,&alpha3,
	    x.arrg,x.strides[0],y.arrgc,y.strides[0],&beta,arrgc,strides[0]); 
#endif	
	  return;
	}
