...
class CFtensorArray {
 ...
 public
  ...
  /*
  CFtensorArray(const int _k, const Gdims& _dims, const vector<int>_strides,
  const int _asize, const int _memsize, const int _cst, const device_id&
  _dev=0): k(_k), dims(_dims), strides(_strides), asize(_asize),
  memsize(_memsize), cst(_cst), device(_dev.id()){
    //CENGINE_DEPRECATED();
    if(device==0){
      arr=new float[memsize];
      arrc=arr+cst;
  ...
  */
  ...
  /*
  CFtensorArray(const int _k, const Gdims& _dims, const vector<int>_strides,
  const int _asize, const int _memsize, const int _cst, const int dev=0): k(_k),
  dims(_dims), strides(_strides), asize(_asize), memsize(_memsize), cst(_cst),
  device(dev){ if(device==0){ arr=new float[memsize]; arrc=arr+cst;
    }
    if(device==1){
      CUDA_SAFE(cudaMalloc((void **)&arrg, memsize*sizeof(float)));
      arrgc=arrg+cst;
 ...
 public
  ...
  /*
  CFtensorArray(const Gdims& _adims, const Gdims& _dims, const fill_identity&
  dummy, const int dev=0): CFtensorArray(_dims,fill::raw,0){
  ...
  */
  ...
  CFtensorArray(const Gdims& _dims,
...
#include "CFtensorArray_access.hpp"
 ...
 public
  ...
  CFtensorArray plus(const CFtensorArray& x) const {
    ...
    CUBLAS_SAFE(
        cublasSaxpy(Cengine_cublas, asize, &alpha, x.arrgc, 1, R.arrgc, 1));
    return R;
  }
  ...
  CFtensorArray elementwise_times(const CFtensorArray& x) const {
    ...
    for (int i = 0; i < asize; i++) {
      R.arrc[i] = arr[i] * x.arrc[i] + arrc[i] * x.arr[i];
    }
    return R;
  }
  ...
  CFtensorArray elementwise_pow(const float p,
                                const complex<float> c = 1.0) const {
    FCG_CPUONLY();
    CFtensorArray R(dims, fill::raw, 0);
    for (int i = 0; i < asize; i++) {
  ...
  float diff2(const CFtensorArray& x) const {
    ...
    for (int i = 0; i < asize; i++) {
      t += (arr[i] - x.arr[i]) * (arr[i] - x.arr[i]) +
           (arrc[i] - x.arrc[i]) * (arrc[i] - x.arrc[i]);
    }
    return t;
  }
 ...
 public
  ...
  void operator+=(const CFtensorArray& x) {
    ...
    CUBLAS_SAFE(
        cublasSaxpy(Cengine_cublas, asize, &alpha, x.arrgc, 1, arrgc, 1));
  }
...
#include "CFtensorArray_add.hpp"
  ...
  void add_ReLU_back(const CFtensorArray& g, const CFtensorArray& x) {
    ...
    for (int i = 0; i < asize; i++) {
      arrc[i] += (x.arrc[i] > 0) * g.arr[i];
    }
  }

  void add_LeakyReLU_back(const CFtensorArray& g,
                          const CFtensorArray& x,
                          const float alpha) {
    assert(x.asize == asize);
    assert(g.asize == asize);
    for (int i = 0; i < asize; i++) {
    ...
    for (int i = 0; i < asize; i++) {
      arrc[i] += ((x.arrc[i] > 0) + (x.arrc[i] <= 0) * alpha) * g.arrc[i];
    }
  }
...
#include "CFtensorArray_add_Mprod.hpp"
 ...
 public
  ...
  string str(const string indent = "") const {
    if (device > 0) {
      return CFtensorArray(*this, 0).str(indent);
    }
    assert(device == 0);
    ostringstream oss;
