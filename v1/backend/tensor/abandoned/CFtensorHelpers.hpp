...
class CFtensorProductType {
 ...
 public:
  CFtensorProductType(int _k,
                      vector<pair<int, int>> _xout,
                      vector<pair<int, int>> _yout,
                      vector<pair<int, int>> _contract,
                      vector<tuple<int, int, int>> _direct)
 ...
 public:
  string str() const {
    ...
    oss << "out: ";
    for (auto p : xout) {
      oss << "x" << p.first << "->" << p.second << " ";
    }
    // oss<<"/ ";
    for (auto p : yout) {
      oss << "y" << p.first << "->" << p.second << " ";
    }
    oss << endl;
    oss << "contract: ";
    for (auto p : contract) {
