...
class CFtensor {
 ...
 public
  ...
  CFtensor(const Gtensor<complex<TYPE>>& x) : CFtensor(x.dims, fill::raw, 0) {
    int dev = x.device;
    x.to_device(0);
    for (int i = 0; i < asize; i++) {
...
#include "CFtensor_add.hpp"
  ...
  void add_LeakyReLU_back(const CFtensor& g,
                          const CFtensor& x,
                          const float alpha) {
    assert(x.asize == asize);
    assert(g.asize == asize);
...
#include "CFtensor_add_Mprod.hpp"
  ...
  */
  ...
  string str(const string indent = "") const {
    if (device > 0) {
      return CFtensor(*this, 0).str(indent);
    }
    ostringstream oss;
...
/*
 CFtensor(const int _k, const Gdims& _dims, const vector<int>_strides, const int
 _asize, const int _memsize, const int _cst, const device_id& _dev=0): k(_k),
...
/*
 inline Gdims CFtensorProductType::dims(const CFtensor& x, const CFtensor& y)
 const{ Gdims d; d.resize(k);
