...
class CscalarObject {
 public:
  ...
  ~CscalarObject() { delete hdl; }

 public
     :  // ---- Filled constructors
        // ------------------------------------------------------------------------
  CscalarObject() { hdl = Cengine_engine->push<new_cscalar_op>(-1, 0); }
 ...
 public
  ...
  CscalarObject(const CscalarObject& x)
      : nbu(x.nbu), hdl(Cengine_engine->push<cscalar_copy_op>(x.hdl)) {}
 ...
 public
  ...
  void flush() const { Cengine_engine->flush(hdl->node); }

 public
     :  // ---- In-place operations
        // ------------------------------------------------------------------------
  void clear() { replace(hdl, Cengine_engine->push<cscalar_set_zero_op>(hdl)); }

  void zero() { replace(hdl, Cengine_engine->push<cscalar_set_zero_op>(hdl)); }
 ...
 public
  ...
  void add(const CscalarObject& x, const float c) {
    replace(hdl,
            Cengine_engine->push<cscalar_add_times_real_op>(hdl, x.hdl, c));
  }

  void add(const CscalarObject& x, const complex<float> c) {
    replace(hdl,
            Cengine_engine->push<cscalar_add_times_complex_op>(hdl, x.hdl, c));
  }
  ...
  void add_div_back0(const CscalarObject& g, const CscalarObject& y) {
    replace(hdl,
            Cengine_engine->push<cscalar_add_div_back0_op>(hdl, g.hdl, y.hdl));
  }

  void add_div_back1(const CscalarObject& g,
                     const CscalarObject& x,
                     const CscalarObject& y) {
    replace(hdl, Cengine_engine->push<cscalar_add_div_back1_op>(hdl, g.hdl,
                                                                x.hdl, y.hdl));
  }
  ...
  void add_abs_back(const CscalarObject& g, const CscalarObject& x) {
    replace(hdl,
            Cengine_engine->push<cscalar_add_abs_back_op>(hdl, g.hdl, x.hdl));
  }
  ...
  void add_pow(const CscalarObject& x,
               const float p,
  ...
  void add_ReLU_back(const CscalarObject& g,
                     const CscalarObject& x,
                     const float c = 0) {
    replace(hdl, Cengine_engine->push<cscalar_add_ReLU_back_op>(hdl, g.hdl,
                                                                x.hdl, c));
  }
  ...
  void add_sigmoid_back(const CscalarObject& g, const CscalarObject& x) {
    replace(hdl, Cengine_engine->push<cscalar_add_sigmoid_back_op>(hdl, g.hdl,
                                                                   x.hdl));
  }
 ...
 public
  ...
  void norm2_into(CscalarObject& R) const { R.add_inp(*this, *this); }
 ...
 public
     :  // ---- I/O
        // ----------------------------------------------------------------------------------------
  string classname() const { return "Cengine::CscalarObject"; }
