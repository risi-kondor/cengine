...
class CtensorObject {
 public:
  ...
  ~CtensorObject() { delete hdl; }

  CtensorObject() {}

  CtensorObject(Chandle* _hdl, const Gdims& _dims) : dims(_dims), hdl(_hdl) {}
  ...
  CtensorObject(const Gdims& _dims,
                const fill_zero& fill,
                const int _device = 0)
      : dims(_dims), device(_device) {
  ...
  CtensorObject(const Gdims& _dims,
                ...
                const int _device = 0)
      : dims(_dims), device(_device) {
    hdl = Cengine_engine->push<new_ctensor_gaussian_op>(_dims, -1, fill.c,
                                                        device);
  }
  ...
  CtensorObject(const Gdims& _dims,
                ...
                const int _device = 0)
      : dims(_dims), nbu(nbd), device(_device) {
    hdl = Cengine_engine->push<new_ctensor_gaussian_op>(_dims, nbd, fill.c,
                                                        device);
  }
 ...
 public
  ...
  CtensorObject(CtensorObject&& x)
      : dims(std::move(x.dims)), nbu(x.nbu), device(x.device) {
    hdl = x.hdl;
    x.hdl = nullptr;
  }
 ...
 public
  ...
  CtensorObject(const Gtensor<complex<float>>& x,
                const fill_tensor& dummy,
                const int _device = 0)
      : dims(x.dims) {
 ...
 public
  ...
  int get_k() const { return dims.size(); }

  Gdims get_dims() const { return dims; }

  int get_dim(const int i) const { return dims[i]; }
  ...
  */
  ...
  void flush() const { ctensor_get(hdl); }
 ...
 public
     :  // ---- In-place operations
        // ------------------------------------------------------------------------
  void clear() { replace(hdl, Cengine_engine->push<ctensor_zero_op>(hdl)); }
 ...
 public
  ...
  CtensorObject transp() const {
    return CtensorObject(Cengine_engine->push<ctensor_transp_op>(hdl), dims,
                         nbu);  // TODO
  }
  ...
  CtensorObject plus(const CtensorObject& x) {
    return CtensorObject(Cengine_engine->push<ctensor_add_op>(hdl, x.hdl, dims),
                         dims, nbu);
  }
 ...
 public
  ...
  void add(const CtensorObject& x, const float c) {
    replace(hdl,
            Cengine_engine->push<ctensor_add_times_real_op>(hdl, x.hdl, c));
  }

  void add(const CtensorObject& x, const complex<float> c) {
    replace(hdl,
            Cengine_engine->push<ctensor_add_times_complex_op>(hdl, x.hdl, c));
  }

  void add(const CtensorObject& x, const RscalarObject& c) {
    replace(hdl,
            Cengine_engine->push<ctensor_add_prod_rA_op>(hdl, c.hdl, x.hdl));
  }

  void add(const CtensorObject& x, const CscalarObject& c) {
    replace(hdl,
            Cengine_engine->push<ctensor_add_prod_cA_op>(hdl, c.hdl, x.hdl));
  }

  void add_cconj(const CtensorObject& x, const CscalarObject& c) {
    replace(hdl,
            Cengine_engine->push<ctensor_add_prod_cc_A_op>(hdl, c.hdl, x.hdl));
  }

  void add_conj(const CtensorObject& x, const CscalarObject& c) {
    replace(hdl,
            Cengine_engine->push<ctensor_add_prod_c_Ac_op>(hdl, c.hdl, x.hdl));
  }
  ...
  void add_Mprod(const CtensorObject& x, const CtensorObject& y) {
    replace(hdl, Cengine_engine->push<ctensor_add_Mprod_op<0, 0>>(
                     hdl, x.hdl, y.hdl, x.dims, y.dims));
  }

  void add_Mprod_AT(const CtensorObject& x, const CtensorObject& y) {
    replace(hdl, Cengine_engine->push<ctensor_add_Mprod_op<2, 0>>(
                     hdl, x.hdl, y.hdl, x.dims, y.dims));
  }

  void add_Mprod_TA(const CtensorObject& x, const CtensorObject& y) {
    replace(hdl, Cengine_engine->push<ctensor_add_Mprod_op<1, 0>>(
                     hdl, x.hdl, y.hdl, x.dims, y.dims));
  }

  void add_Mprod_AC(const CtensorObject& x, const CtensorObject& y) {
    replace(hdl, Cengine_engine->push<ctensor_add_Mprod_op<0, 2>>(
                     hdl, x.hdl, y.hdl, x.dims, y.dims));
  }

  void add_Mprod_TC(const CtensorObject& x, const CtensorObject& y) {
    replace(hdl, Cengine_engine->push<ctensor_add_Mprod_op<1, 2>>(
                     hdl, x.hdl, y.hdl, x.dims, y.dims));
  }

  void add_Mprod_AH(const CtensorObject& x, const CtensorObject& y) {
    replace(hdl, Cengine_engine->push<ctensor_add_Mprod_op<2, 2>>(
                     hdl, x.hdl, y.hdl, x.dims, y.dims));
  }

  void add_Mprod_HA(const CtensorObject& x, const CtensorObject& y) {
    replace(hdl, Cengine_engine->push<ctensor_add_Mprod_op<1, 1>>(
                     hdl, x.hdl, y.hdl, x.dims, y.dims));
  }
  ...
  void add_ReLU_back(const CtensorObject& g,
                     const CtensorObject& x,
                     const float c = 0) {
    replace(hdl, Cengine_engine->push<ctensor_add_ReLU_back_op>(hdl, g.hdl,
                                                                x.hdl, c));
  }
 ...
 public
  ...
  CtensorObject divide_columns(const CtensorObject& N) {
    return CtensorObject(
        Cengine_engine->push<ctensor_divide_cols_op>(hdl, N.hdl), dims, nbu);
  }

 public
     :  // ---- I/O
        // ----------------------------------------------------------------------------------------
  string classname() const { return "GEnet::CtensorObject"; }
...
CtensorObject operator*(const Transpose<CtensorObject>& x,
                        const CtensorObject& y) {
